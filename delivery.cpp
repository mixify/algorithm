...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = i; j < N; ++j) {
            mid = stuff[(i+j)/2];
            if(i==j) {summation[i][j] = stuff[i]*pay_truck; summation_heli[i][j] = abs(stuff[i]-mid) + pay_heli;}
            else
            {
                summation[i][j] = summation[i][j-1] + stuff[j]*pay_truck;
                summation_heli[i][j] = summation_heli[i][j-1] + abs(stuff[j]-mid)*pay_truck;
            }
        }
    }
